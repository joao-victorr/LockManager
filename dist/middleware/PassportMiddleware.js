...
passport_1.default.use(new passport_jwt_1.Strategy(options, async (payload, done) => {
    const user = await PrismaClient_1.prismaClient.usersWeb.findUnique({ where: { id: payload.id } });
...
const generateToken = (data) => {
    return jsonwebtoken_1.default.sign({ id: data }, process.env.JWT_SECRET, { expiresIn: '24h' });
};
